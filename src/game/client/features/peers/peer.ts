...
export class Peer extends Phaser.GameObjects.Sprite {
    ...
    constructor(config: PeerConfig) {
        ...
        super(config.scene, config.playerData.position.x, config.playerData.position.y, "soldier", 0);
        ...
        this.targetPosition = { x: config.playerData.position.x, y: config.playerData.position.y };
        ...
        console.log(`Created peer sprite for ${this.playerData.name} at (${this.x}, ${this.y}) with scale ${PLAYER_CONFIG.movement.scale}`);
    }
    ...
    private playAnimation(animationKey: string): void {
        if (this.scene.anims.exists(animationKey) && this.currentAnimation !== animationKey) {
    ...
    public updateFromData(playerData: PlayerData): void {
        ...
        if (targetDistance > 0.5) {
            ...
            if (currentDistance > 150) {
                ...
                this.targetPosition = { x: newTargetX, y: newTargetY };
                console.log(`Updated peer ${this.playerData.name} target to (${newTargetX}, ${newTargetY})`);
            }
        }
    }
