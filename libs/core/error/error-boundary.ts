...
export class ErrorBoundary {
  ...
  private setupGlobalErrorHandlers(): void {
    ...
    window.addEventListener('error', (event) => {
      this.handleError(
        new GameError(`Uncaught error: ${event.message}`, ErrorSeverity.HIGH, {
          action: 'global-error',
        })
      );
    ...
    window.addEventListener('unhandledrejection', (event) => {
      this.handleError(
        new GameError(`Unhandled promise rejection: ${event.reason}`, ErrorSeverity.HIGH, {
   ...
   */
  public wrap<T extends (...args: any[]) => any>(fn: T, context: ErrorContext): T {
