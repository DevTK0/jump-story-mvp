        ...
        try {
            ...
            if (scene.player && scene.player.anims) {
                try {
                    ...
                    // Ignore if animation doesn't exist
                }
            }
...
export class NetworkReconnectionStrategy implements ErrorRecoveryStrategy {
    ...
    async recover(_error: Error, context: ErrorContext): Promise<void> {
        ...
        this.logger.info(`Attempting network reconnection (attempt ${this.reconnectAttempts}/${this.MAX_RECONNECT_ATTEMPTS})`);
        ...
        try {
            ...
            this.logger.error(`Network reconnection failed (attempt ${this.reconnectAttempts})`, { error: recoveryError });
