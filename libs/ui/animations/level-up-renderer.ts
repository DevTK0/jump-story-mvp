...
export class LevelUpRenderer {
  ...
  public showLevelUpAnimation(playerIdentity: Identity, _newLevel: number): void {
    ...
    const updateHandler = () => {
      ...
      if (sprite && sprite.active) {
        levelUpText.setPosition(sprite.x, sprite.y + PLAYER_STATS_UI_CONFIG.levelUp.offset.y);
      } else if (this.dbConnection) {
        // Fallback to database position
        for (const p of this.dbConnection.db.player.iter()) {
          if (p.identity.toHexString() === playerIdentityHex) {
            levelUpText.setPosition(p.x, p.y + PLAYER_STATS_UI_CONFIG.levelUp.offset.y);
            break;
          }
        }
      }
  ...
  private createLevelUpText(x: number, y: number): Phaser.GameObjects.Text {
    ...
    // Create text first
    const levelUpText = this.scene.add.text(x, y + config.offset.y, config.text, config.style);
