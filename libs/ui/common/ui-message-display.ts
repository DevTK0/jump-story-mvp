...
export interface UIMessageConfig {
  fontSize: string;
  fontFamily: string;
  color: string;
...
const DEFAULT_CONFIG: UIMessageConfig = {
  fontSize: '18px',
...
export class UIMessageDisplay {
  private scene: Phaser.Scene;
  private camera: Phaser.Cameras.Scene2D.Camera;
  private container: Phaser.GameObjects.Container;
  private background: Phaser.GameObjects.Rectangle;
  private text: Phaser.GameObjects.Text;
  private config: UIMessageConfig;
  private hideTimer?: Phaser.Time.TimerEvent;

  constructor(scene: Phaser.Scene, config?: Partial<UIMessageConfig>) {
    this.scene = scene;
    ...
    this.background = scene.add.rectangle(0, 0, 100, 30, 0x000000, 0.8);
  ...
  private updatePosition(): void {
    ...
    this.container.setPosition(width / 2, height * 0.25);
  }
