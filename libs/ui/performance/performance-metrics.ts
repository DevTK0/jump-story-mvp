...
export class PerformanceMetrics {
  ...
  // Update tracking
  private updateInterval: number = UI_CONFIG.performanceMetrics.updateInterval;
  ...
  constructor(scene: Phaser.Scene, config?: PerformanceMetricsConfig) {
    ...
    const x = config?.x ?? UI_CONFIG.performanceMetrics.position.x;
    const y = config?.y ?? UI_CONFIG.performanceMetrics.position.y;
    ...
    this.container.setDepth(UI_CONFIG.depth.performanceMetrics);

    // Create background
    this.background = scene.add.rectangle(
      0,
      0,
      UI_CONFIG.performanceMetrics.panel.width,
      UI_CONFIG.performanceMetrics.panel.height,
