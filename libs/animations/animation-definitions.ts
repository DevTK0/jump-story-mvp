...
export const ANIMATION_DEFINITIONS = {
  ...
  orc: {
    ...
    death: { start: 50, end: 58, frameRate: 12 }, // Example death animation
  } satisfies SpriteAnimationSet,

  /**
   * Example additional enemy type
   */
  goblin: {
    idle: { start: 0, end: 3, frameRate: 6 },
    walk: { start: 8, end: 15, frameRate: 10 },
    attack1: { start: 16, end: 21, frameRate: 15 },
    hit: { start: 24, end: 28, frameRate: 12 },
    death: { start: 32, end: 39, frameRate: 10 },
  } satisfies SpriteAnimationSet,

  /**
   * NPC animations (for future use)
   */
  villager: {
    idle: { start: 0, end: 7, frameRate: 4 },
...
export const ANIMATION_BEHAVIORS = {
  ...
  ONE_SHOT_ANIMATIONS: ['attack1', 'attack2', 'attack3', 'hurt', 'hit', 'death'] as const,
  
  // Which animations can be interrupted
  INTERRUPTIBLE_ANIMATIONS: ['idle', 'walk', 'run'] as const,
  
  // Which animations cannot be interrupted
  NON_INTERRUPTIBLE_ANIMATIONS: ['attack1', 'attack2', 'attack3', 'hurt', 'death'] as const,
 ...
 */
export function getAnimationDefinition(spriteKey: string): SpriteAnimationSet | undefined {
  return ANIMATION_DEFINITIONS[spriteKey as keyof typeof ANIMATION_DEFINITIONS];
}
...
export function getAnimationPriority(animationType: string): number {
  return ANIMATION_BEHAVIORS.ANIMATION_PRIORITIES[animationType as keyof typeof ANIMATION_BEHAVIORS.ANIMATION_PRIORITIES] ?? 1;
}